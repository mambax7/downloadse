...
if($HTTP_POST_VARS['submit']) {
	...
	$newid = $xoopsDB->genId($xoopsDB->prefix("downloadse_mod")."_requestid_seq");

	$sql = sprintf("INSERT INTO %s (requestid, lid, cid, title, url, homepage, version, size, platform, logourl, description, modifysubmitter) VALUES (%u, %u, %u, '%s', '%s', '%s', '%s', %u, '%s', '%s', '%s', %u)", $xoopsDB->prefix("downloadse_mod"), $newid, $lid, $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, $description, $ratinguser);
	...
	$xoopsOption['template_main'] = 'downloadse_modfile.html';
	include XOOPS_ROOT_PATH."/header.php";
	$result = $xoopsDB->query("SELECT cid, title, url, homepage, version, size, platform, logourl FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE lid=".$lid." AND status>0");
