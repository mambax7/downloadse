...
if ( $xoopsModuleConfig['check_host'] ) {
	...
	if (!$goodhost) {
		redirect_header(XOOPS_URL . "/modules/downloadse/singlefile.php?cid=$cid&amp;lid=$lid", 20, _MD_NOPERMISETOLINK);
		exit();
	}
}
$sql = sprintf("UPDATE %s SET hits = hits+1 WHERE lid = %u AND status > 0", $xoopsDB->prefix("downloadse_downloads"), $lid);
$xoopsDB->queryF($sql);
$result = $xoopsDB->query("SELECT url FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE lid=$lid AND status>0");
