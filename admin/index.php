...
$eh = new ErrorHandler;
$mytree = new XoopsTree($xoopsDB->prefix("downloadse_cat"),"cid","pid");
...
function downloadse()
{
	...
	$result = $xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_broken")."");
	...
	$result2 = $xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_mod")."");
	...
	$result3 = $xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE status=0");
	...
	$result=$xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE status>0");
...
function listNewDownloads()
{
	...
	$downimg_array = XoopsLists::getImgListAsArray(XOOPS_ROOT_PATH."/modules/downloadse/images/shots/");
	$result = $xoopsDB->query("SELECT lid, cid, title, url, homepage, version, size, platform, logourl, submitter FROM ".$xoopsDB->prefix("downloadse_downloads")." where status=0 ORDER BY date DESC");
	...
	if ($numrows>0) {
		while(list($lid, $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, $uid) = $xoopsDB->fetchRow($result)) {
			$result2 = $xoopsDB->query("SELECT description FROM ".$xoopsDB->prefix("downloadse_text")." WHERE lid=$lid");
			...
			$directory = XOOPS_URL."/modules/downloadse/images/shots/";
...
function downloadsConfigMenu()
{
	...
	$result=$xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_cat")."");
	list($numrows)=$xoopsDB->fetchRow($result);
	if($numrows>0) {
                echo"<table width='100%' border='0' cellspacing='1' class='outer'>"
                   ."<tr class=\"odd\"><td>";
	...
	// Modify Download
	$result2 = $xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_downloads")."");
	...
	if ($numrows2>0) {
		...
		echo "<input type=hidden name=fct value=downloadse>\n";
...
function modDownload()
{
	...
	echo "<h4>"._MD_DLCONF."</h4>";
        echo"<table width='100%' border='0' cellspacing='1' class='outer'>"
           ."<tr class=\"odd\"><td>";
	$result = $xoopsDB->query("SELECT cid, title, url, homepage, version, size, platform, logourl FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE lid=$lid") or $eh->show("0013");
	echo "<h4>"._MD_MODDL."</h4><br />";
	...
	$result5=$xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_votedata")."");
	...
	$result5=$xoopsDB->query("SELECT ratingid, ratinguser, rating, ratinghostname, ratingtimestamp FROM ".$xoopsDB->prefix("downloadse_votedata")." WHERE lid = $lid AND ratinguser != 0 ORDER BY ratingtimestamp DESC");
	...
	while(list($ratingid, $ratinguser, $rating, $ratinghostname, $ratingtimestamp)=$xoopsDB->fetchRow($result5)) {
		...
		$result2=$xoopsDB->query("SELECT rating FROM ".$xoopsDB->prefix("downloadse_votedata")." WHERE ratinguser = $ratinguser");
	...
	$result5=$xoopsDB->query("SELECT ratingid, rating, ratinghostname, ratingtimestamp FROM ".$xoopsDB->prefix("downloadse_votedata")." WHERE lid = $lid AND ratinguser = 0 ORDER BY ratingtimestamp DESC");
	$votes = $xoopsDB->getRowsNum($result5);
...
function delVote()
{
	...
	$lid = $HTTP_GET_VARS['lid'];
	$sql = sprintf("DELETE FROM %s WHERE ratingid = %u", $xoopsDB->prefix("downloadse_votedata"), $rid);
...
{
	global $xoopsDB, $eh;
	$result = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("downloadse_broken")." ORDER BY reportid");
	...
	if ($totalbrokendownloads==0) {
		...
		while(list($reportid, $lid, $sender, $ip)=$xoopsDB->fetchRow($result)){
			$result2 = $xoopsDB->query("SELECT title, url, submitter FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE lid=$lid");
			if ($sender != 0) {
...
function delBrokenDownloads()
{
	global $xoopsDB, $HTTP_GET_VARS, $eh;
	$lid = $HTTP_GET_VARS['lid'];
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_broken"), $lid);
	$xoopsDB->query($sql) or $eh->show("0013");
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_downloads"), $lid);
	$xoopsDB->query($sql) or $eh->show("0013");
...
{
	global $xoopsDB, $HTTP_GET_VARS, $eh;
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_broken"), $HTTP_GET_VARS['lid']);
	$xoopsDB->query($sql) or $eh->show("0013");
...
function listModReq()
{
	...
	$result = $xoopsDB->query("SELECT * FROM ".$xoopsDB->prefix("downloadse_mod")." ORDER BY requestid");
	...
	if($totalmodrequests>0){
		...
		while(list($requestid, $lid, $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, $description, $modifysubmitter)=$xoopsDB->fetchRow($result)) {
			...
			$result2 = $xoopsDB->query("SELECT cid, title, url, homepage, version, size, platform, logourl, submitter FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE lid=$lid");
			...
			if ( $xoopsModuleConfig['useshots'] && !empty($origlogourl) ){
				echo "<img src=\"".XOOPS_URL."/modules/downloadse/images/shots/".$origlogourl."\" width=\"".$xoopsModuleConfig['shotwidth']."\">";
			}else{
				echo "&nbsp;";
			}
			echo "</td></tr>
			...
			if ( $xoopsModuleConfig['useshots'] && !empty($logourl) ){
				echo "<img src=\"".XOOPS_URL."/modules/downloadse/images/shots/".$logourl."\" width=\"".$xoopsModuleConfig['shotwidth']."\">";
			} else {
				echo "&nbsp;";
			}
			echo "</td></tr>
...
function changeModReq()
{
	...
	$query = "SELECT lid, cid, title, url, homepage, version, size, platform, logourl, description FROM ".$xoopsDB->prefix("downloadse_mod")." WHERE requestid=$requestid";
	...
	while(list($lid, $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, $description)=$xoopsDB->fetchRow($result)) {
		...
		$sql = sprintf("UPDATE %s SET cid = %u,title = '%s', url = '%s', homepage = '%s', version = '%s', size = %u, platform = '%s', logourl = '%s', status = %u, date = %u WHERE lid = %u", $xoopsDB->prefix("downloadse_downloads"), $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, 2, time(), $lid);
		$xoopsDB->query($sql) or $eh->show("0013");
		$sql = sprintf("UPDATE %s SET description = '%s' WHERE lid = %u", $xoopsDB->prefix("downloadse_text"), $description, $lid);
		$xoopsDB->query($sql) or $eh->show("0013");
		$sql = sprintf("DELETE FROM %s WHERE requestid = %u", $xoopsDB->prefix("downloadse_mod"), $requestid);
		$xoopsDB->query($sql) or $eh->show("0013");
	}
...
{
	global $xoopsDB, $HTTP_GET_VARS, $eh;
	$sql = sprintf("DELETE FROM %s WHERE requestid = %u", $xoopsDB->prefix("downloadse_mod"), $HTTP_GET_VARS['requestid']);
	$xoopsDB->query($sql) or $eh->show("0013");
...
function modDownloadS()
{
	...
	$sql = sprintf("UPDATE %s SET cid = %u, title = '%s', url = '%s', homepage = '%s', version = '%s', size = %u, platform = '%s', logourl = '%s', status = %u, date = %u WHERE lid = %u", $xoopsDB->prefix("downloadse_downloads"), $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, 2, time(), $HTTP_POST_VARS['lid']);
	$xoopsDB->query($sql)  or $eh->show("0013");
	$sql = sprintf("UPDATE %s SET description = '%s' WHERE lid = %u", $xoopsDB->prefix("downloadse_text"), $description, $HTTP_POST_VARS['lid']);
	$xoopsDB->query($sql)  or $eh->show("0013");
	redirect_header("index.php",1,_MD_DBUPDATED);
}
...
function delDownload()
{
	...
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_downloads"), $HTTP_GET_VARS['lid']);
	$xoopsDB->query($sql) or $eh->show("0013");
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_text"), $HTTP_GET_VARS['lid']);
	$xoopsDB->query($sql) or $eh->show("0013");
	$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_votedata"), $HTTP_GET_VARS['lid']);
	$xoopsDB->query($sql) or $eh->show("0013");
	// delete comments
...
function modCat()
{
	...
	$result=$xoopsDB->query("SELECT pid, title, imgurl FROM ".$xoopsDB->prefix("downloadse_cat")." WHERE cid=$cid");
...
function modCatS()
{
	...
	$sql = sprintf("UPDATE %s SET title = '%s', imgurl = '%s', pid = %u WHERE cid = %u", $xoopsDB->prefix("downloadse_cat"), $title, $imgurl, $sid, $cid);
	$xoopsDB->query($sql) or $eh->show("0013");
	redirect_header("index.php",1,_MD_DBUPDATED);
}
...
function delCat()
{
	...
	if ($ok == 1) {
		...
		for ($i = 0; $i < $lcount; $i++) {
			...
			$result=$xoopsDB->query("SELECT lid FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE cid=".$arr[$i]."") or $eh->show("0013");
			...
			while ( list($lid)=$xoopsDB->fetchRow($result) ) {
				$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_text"), $lid);
				$xoopsDB->query($sql) or $eh->show("0013");
				$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_votedata"), $lid);
				$xoopsDB->query($sql) or $eh->show("0013");
				$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_downloads"), $lid);
				$xoopsDB->query($sql) or $eh->show("0013");
				// delete comments
			...
			$sql = sprintf("DELETE FROM %s WHERE cid = %u", $xoopsDB->prefix("downloadse_cat"), $arr[$i]);
			$xoopsDB->query($sql) or $eh->show("0013");
		}
		//all subcategory and associated data are deleted, now delete category data and its associated data
		$result=$xoopsDB->query("SELECT lid FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE cid=".$cid."") or $eh->show("0013");
		while(list($lid)=$xoopsDB->fetchRow($result)){
			$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_downloads"), $lid);
			$xoopsDB->query($sql) or $eh->show("0013");
			$sql = sprintf("DELETE FROM %s WHERE lid = %u", $xoopsDB->prefix("downloadse_text"), $lid);
		...
		$sql = sprintf("DELETE FROM %s WHERE cid = %u", $xoopsDB->prefix("downloadse_cat"), $cid);
		$xoopsDB->query($sql) or $eh->show("0013");
...
function addCat()
{
	...
	$newid = $xoopsDB->genId($xoopsDB->prefix("downloadse_cat")."_cid_seq");
	$sql = sprintf("INSERT INTO %s (cid, pid, title, imgurl) VALUES (%u, %u, '%s', '%s')", $xoopsDB->prefix("downloadse_cat"), $newid, $pid, $title, $imgurl);
	$xoopsDB->query($sql) or $eh->show("0013");
	if ($newid == 0) {
...
function addDownload()
{
	...
	$result = $xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE url='$url'");
	list($numrows) = $xoopsDB->fetchRow($result);
	$error = 0;
	$errormsg = "";
	if ($numrows>0) {
	...
	$newid = $xoopsDB->genId($xoopsDB->prefix("downloadse_downloads")."_lid_seq");

	$sql = sprintf("INSERT INTO %s (lid, cid, title, url, homepage, version, size, platform, logourl, submitter, status, date, hits, rating, votes, comments) VALUES (%u, %u, '%s', '%s', '%s', '%s', %u, '%s', '%s', %u, %u, %u, %u, %u, %u, %u)", $xoopsDB->prefix("downloadse_downloads"), $newid, $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, $submitter, 1, time(), 0, 0, 0, 0);
	$xoopsDB->query($sql) or $eh->show("0013");
	if( $newid == 0 ) {
		$newid = $xoopsDB->getInsertId();
	}
	$sql = sprintf("INSERT INTO %s (lid, description) VALUES (%u, '%s')", $xoopsDB->prefix("downloadse_text"), $newid, $description);
	$xoopsDB->query($sql) or $eh->show("0013");
	$tags = array();
	$tags['FILE_NAME'] = $title;
	$tags['FILE_URL'] = XOOPS_URL . '/modules/' . $xoopsModule->getVar('dirname') . '/singlefile.php?cid=' . $cid . '&amp;lid=' . $newid;
	$sql = "SELECT title FROM " . $xoopsDB->prefix("downloadse_cat") . " WHERE cid=" . $cid;
...
function approve()
{
	...
	$sql = sprintf("UPDATE %s SET cid = %u, title = '%s', url = '%s', homepage = '%s', version = '%s', size = %u, platform = '%s', logourl = '%s', status = %u, date = %u WHERE lid = %u", $xoopsDB->prefix("downloadse_downloads"), $cid, $title, $url, $homepage, $version, $size, $platform, $logourl, 1, time(), $lid);
	$xoopsDB->query($sql) or $eh->show("0013");
	$sql = sprintf("UPDATE %s SET description = '%s' WHERE lid = %u", $xoopsDB->prefix("downloadse_text"), $description, $lid);
	$xoopsDB->query($sql) or $eh->show("0013");
	global $xoopsModule;
