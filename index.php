...
$xoopsOption['template_main'] = 'downloadse_index.html';
include XOOPS_ROOT_PATH."/header.php";

$sql = "SELECT cid, title, imgurl FROM ".$xoopsDB->prefix("downloadse_cat")." WHERE pid = 0 ORDER BY title";
...
while($myrow = $xoopsDB->fetchArray($result)) {
	...
	foreach($arr as $ele){
		...
		$subcategories .= "<a href=\"".XOOPS_URL."/modules/downloadse/viewcat.php?cid=".$ele['cid']."\">".$chtitle."</a>";
		$space++;
		$chcount++;
	}	
...
list($numrows)=$xoopsDB->fetchRow($xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE status>0"));
...
$result = $xoopsDB->query("SELECT d.lid, d.cid, d.title, d.url, d.homepage, d.version, d.size, d.platform, d.logourl, d.status, d.date, d.hits, d.rating, d.votes, d.comments, t.description FROM ".$xoopsDB->prefix("downloadse_downloads")." d, ".$xoopsDB->prefix("downloadse_text")." t WHERE d.status>0 AND d.lid=t.lid ORDER BY date DESC", $xoopsModuleConfig['newdownloads'], 0);
while(list($lid, $cid, $dtitle, $url, $homepage, $version, $size, $platform, $logourl, $status, $time, $hits, $rating, $votes, $comments, $description)=$xoopsDB->fetchRow($result)) {
	...
	$path = substr($path, 1);
	$path = str_replace("/"," <img src='".XOOPS_URL."/modules/downloadse/images/arrow.gif' board='0' alt=''> ",$path);
	...
	$pop = popgraphic($hits);
	if ($isadmin) {
		$adminlink = '<a href="'.XOOPS_URL.'/modules/downloadse/admin/index.php?lid='.$lid.'&fct=downloadse&op=modDownload"><img src="'.XOOPS_URL.'/modules/downloadse/images/editicon.gif" border="0" alt="'._MD_EDITTHISDL.'" /></a>';
	} else {
		$adminlink = '';
	}
	if ($votes == 1) {
		...
		$xoopsTpl->append('file', array('id' => $lid,'cid'=>$cid,'rating' => $rating,'title' => $dtitle.$new.$pop,'logourl' => $logourl,'updated' => $datetime,'description' => $description,'adminlink' => $adminlink,'hits' => $hits,'votes' => $votestring, 'comments' => $comments, 'platform' => $platform,'size'  => $size,'homepage' => $homepage,'version'  => $version,'category'  => $path,'lang_dltimes' => sprintf(_MD_DLTIMES,$hits),'mail_subject' => rawurlencode(sprintf(_MD_INTFILEFOUND,$xoopsConfig['sitename'])),'mail_body' => rawurlencode(sprintf(_MD_INTFILEFOUND,$xoopsConfig['sitename']).':  '.XOOPS_URL.'/modules/downloadse/singlefile.php?cid='.$cid.'&amp;lid='.$lid)));
}

include XOOPS_ROOT_PATH."/modules/downloadse/footer.php";
?>
