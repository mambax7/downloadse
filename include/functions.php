...
function mainheader($mainlink=1) {
		echo "<br /><br /><p><a href=\"".XOOPS_URL."/modules/downloadse/index.php\"><img src=\"".XOOPS_URL."/modules/downloadse/images/logo-en.gif\" border=\"0\" alt\"\" /></a></p><br /><br />";
}
...
function newdownloadgraphic($time, $status) {
	...
	$startdate = (time()-(86400 * $count));
		if ($startdate < $time) {
		if($status==1){
			$new = "&nbsp;<img src=\"".XOOPS_URL."/modules/downloadse/images/newred.gif\" alt=\""._MD_NEWTHISWEEK."\" />";
		}elseif($status==2){
			$new = "&nbsp;<img src=\"".XOOPS_URL."/modules/downloadse/images/update.gif\" alt=\""._MD_UPTHISWEEK."\" />";
		}
	}
	return $new;
}
...
function popgraphic($hits) {
	...
	if ($hits >= $xoopsModuleConfig['popular']) {
		return "&nbsp;<img src =\"".XOOPS_URL."/modules/downloadse/images/pop.gif\" alt=\""._MD_POPULAR."\" />";
	}
	return '';
}
...
function updaterating($sel_id){
	global $xoopsDB;
	$query = "select rating FROM ".$xoopsDB->prefix("downloadse_votedata")." WHERE lid = ".$sel_id."";
	...
	$sql = sprintf("UPDATE %s SET rating = %u, votes = %u WHERE lid = %u", $xoopsDB->prefix("downloadse_downloads"), $finalrating, $votesDB, $sel_id);
	$xoopsDB->query($sql);
}
...
function getTotalItems($sel_id, $status=""){
	...
	$count = 0;
	$arr = array();
	$query = "select count(*) from ".$xoopsDB->prefix("downloadse_downloads")." where cid=".$sel_id."";
	if($status!=""){
	...
	$size = count($arr);
	for($i=0;$i<$size;$i++){
		$query2 = "select count(*) from ".$xoopsDB->prefix("downloadse_downloads")." where cid=".$arr[$i]."";
		if($status!=""){
