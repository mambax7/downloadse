...
if (!empty($HTTP_POST_VARS['submit'])) {
	...
	if ( $sender != 0 ) {
		...
		$result=$xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_broken")." WHERE lid=$lid AND sender=$sender");
		...
		$result=$xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_broken")." WHERE lid=$lid AND ip = '$ip'");
		list ($count)=$xoopsDB->fetchRow($result);
		if ( $count > 0 ) {
			redirect_header("index.php",2,_MD_ALREADYREPORTED);
			exit();
	...
	$newid = $xoopsDB->genId($xoopsDB->prefix("downloadse_broken")."_reportid_seq");
    $sql = sprintf("INSERT INTO %s (reportid, lid, sender, ip) VALUES (%u, %u, %u, '%s')", $xoopsDB->prefix("downloadse_broken"), $newid, $lid, $sender, $ip);
	...
	$xoopsOption['template_main'] = 'downloadse_brokenfile.html';
