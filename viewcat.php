...
$xoopsOption['template_main'] = 'downloadse_viewcat.html';
...
if ( count($arr) > 0 ) {
	...
	foreach($arr as $ele){
		...
		foreach($sub_arr as $sub_ele){
			...
			$infercategories .= "<a href=\"".XOOPS_URL."/modules/downloadse/viewcat.php?cid=".$sub_ele['cid']."\">".$chtitle."</a>";
			$space++;
			$chcount++;
		}
...
$fullcountresult=$xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE cid=$cid AND status>0");
...
if($numrows>0){
	...
	$q = "SELECT d.lid, d.title, d.url, d.homepage, d.version, d.size, d.platform, d.logourl, d.status, d.date, d.hits, d.rating, d.votes, d.comments, t.description FROM ".$xoopsDB->prefix("downloadse_downloads")." d, ".$xoopsDB->prefix("downloadse_text")." t WHERE cid=".$cid." AND d.status>0 AND d.lid=t.lid ORDER BY ".$orderby."";
