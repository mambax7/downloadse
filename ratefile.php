...
if(!empty($HTTP_POST_VARS['submit'])) {
	...
	if ($ratinguser != 0) {
		$result=$xoopsDB->query("SELECT submitter FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE lid=$lid");
		...
		$result=$xoopsDB->query("SELECT ratinguser FROM ".$xoopsDB->prefix("downloadse_votedata")." WHERE lid=$lid");
		while(list($ratinguserDB)=$xoopsDB->fetchRow($result)) {
			if ($ratinguserDB==$ratinguser) {
		...
		$result=$xoopsDB->query("SELECT COUNT(*) FROM ".$xoopsDB->prefix("downloadse_votedata")." WHERE lid=$lid AND ratinguser=0 AND ratinghostname = '$ip'  AND ratingtimestamp > $yesterday");
	...
	$newid = $xoopsDB->genId($xoopsDB->prefix("downloadse_votedata")."_ratingid_seq");
	$datetime = time();
	$sql = sprintf("INSERT INTO %s (ratingid, lid, ratinguser, rating, ratinghostname, ratingtimestamp) VALUES (%u, %u, %u, %u, '%s', %u)", $xoopsDB->prefix("downloadse_votedata"), $newid, $lid, $ratinguser, $rating, $ip, $datetime);
	...
	$xoopsOption['template_main'] = 'downloadse_ratefile.html';
    include XOOPS_ROOT_PATH."/header.php";
    $lid = intval($HTTP_GET_VARS['lid']);
    $result=$xoopsDB->query("SELECT title FROM ".$xoopsDB->prefix("downloadse_downloads")." WHERE lid=$lid");
